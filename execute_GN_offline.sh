...
declare -r DATADIR="./experiment_data/Q_values"
        ...
        if [[ ${cfg} == *"Q_matrix"* ]]; then
	        echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>${cfg}"
				...
				if [ "$APPLICATION" == "ones-solvers-cg" ]; then
		  			python GN_OFFLINE_RL_HW.py max-range-config.yaml ones-solvers-cg 10000 poor 0 10000 ${cfg}
                elif [ "$APPLICATION" == "ones-solvers-bicgstab" ]; then
		  			python GN_OFFLINE_RL_HW.py max-range-config.yaml ones-solvers-bicgstab 20000 poor 0 10000 ${cfg}
                else
                	python GN_OFFLINE_RL_HW.py max-range-config.yaml -- ${APPLICATION} ${PROBLEM_SIZE} ${ITERATION_COUNT} ${cfg}
				fi
