...
class PIController:
    ...
    def control(self, csvwriters, argument):
        ...
        while not self.daemon.all_finished():
            ...
            if msg_type == 'pubProgress':
                ...
                self._update_measure(payload,argument)
            else:
                continue
                print("missed")
    ...
    def _update_measure(self, payload,argument):
        # print(argument)
        timestamp, measures = payload
        timestamp *= 1e-6  # convert µs in s
...
def launch_application(config, daemon_cfg, workload_cfg,argument, *, sleep_duration=0.5):
    # print(argument)
    with nrm.nrmd(daemon_cfg) as daemon:
        ...
        with contextlib.ExitStack() as stack:
            ...
            controller.control(csvwriters, argument)
...
def run(options, cmd):
    ...
    launch_application(options.config, daemon_cfg, workload_cfg, argument)
