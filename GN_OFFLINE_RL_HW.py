...
APPLICATIONS = ['ones-stream-scale', 'ones-solvers-cg', 'ones-stream-copy', 'ones-stream-add', 'ones-solvers-bicgstab', 'ones-stream-triad', 'ones-stream-full', 'ones-npb-ep']
NUM_APPS = len(APPLICATIONS)
TOTAL_OBS = NUM_APPS*TOT_PRO


def idx_to_xy(idx):
    if hasattr(idx, '__len__'):  # array
        x = idx % TOT_PRO
        y = np.floor(idx/TOT_PRO).astype(np.int32)
        xy = np.c_[x,y]
        return xy
    else:
        return np.array([ idx % TOT_PRO, int(np.floor(idx/TOT_PRO))])

def xy_to_idx(key):
    shape = np.array(key).shape
    if len(shape) == 1:
        return key[0] + key[1]*TOT_PRO
    elif len(shape) == 2:
        return key[:,0] + key[:,1]*TOT_PRO
    else:
...
class PIController:
    ...
    def control(self, csvwriters, model_file, APP_ID):
        ...
        while not self.daemon.all_finished():
            ...
            if msg_type == 'pubProgress':
                ...
                self._update_measure(payload, APP_ID)
            else:
                print("continuing")
                continue
    ...
    def _update_measure(self, payload, APP_ID):
        timestamp, measures = payload
        timestamp *= 1e-6  # convert µs in s
        for data in measures:
            if data['sensorID'].startswith('RaplKey'):
                ...
                new_obs = round(obs)
                obs = xy_to_idx(np.array([new_obs,APP_ID]))
                # print(obs)
                action_space = self.model[obs,:]
...
def launch_application(config, daemon_cfg, workload_cfg, RLM, APP_ID, *, sleep_duration=0.5):
    # print(">>>>>>>>>>>>>>>>>>>>",APP_ID, RLM)
    with nrm.nrmd(daemon_cfg) as daemon:
        ...
        with contextlib.ExitStack() as stack:
            ...
            controller.control(csvwriters, RLM, APP_ID)
...
def run(options, cmd):
    RLM = cmd[-1]
    WORKLOAD = cmd[0]
    APP_ID = APPLICATIONS.index(WORKLOAD)
    ...
    launch_application(options.config, daemon_cfg, workload_cfg, RLM, APP_ID)
